...
async def generate_cards_route(data: GenerateCardsRequest, request: FastAPIRequest):
    ...
    if use_ai:
        ...
        # Generate fallback color name directly here when AI is disabled
        hex_clean = data.hexColor.lstrip('#').upper()
...
async def generate_card_details_route(data: CardDetailsRequest, request: FastAPIRequest):
    ...
    try:
        ...
        if not hex_color:
            raise HTTPException(status_code=400, detail="Missing required data: hexColor")
