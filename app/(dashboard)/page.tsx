...
export default function HomePage() {
  ...
  const handleGenerateImageClick = async () => {
    ...
    try {
      // Send only the minimum required data to the backend
      ...
      // Parse the JSON response immediately to handle errors properly
      const resultData = await response.json();
      
      // Check for errors in the response
      if (!response.ok) {
        throw new Error(resultData.error || `Card generation failed with status: ${response.status}`);
      }
