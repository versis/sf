...
export default function HomePage() {
  ...
  const handleGenerateImageClick = async () => {
    ...
    try {
      // Single API call to generate all cards
      const response = await fetch('/api/generate-cards', {
        ...
        body: JSON.stringify({
          ...
          // cardName, phoneticName, article, description are now handled by the backend if not provided
          // cardId is also handled by the backend or uses a default
        }),
      });
      ...
      // console.log("Received data from /api/generate-cards:", resultData);

      if (resultData.error) {
        throw new Error(resultData.error);
      }

      if (!resultData.generated_cards || resultData.generated_cards.length === 0) {
        throw new Error('No cards were generated by the API.');
      }
      
      // AI details are now part of resultData.ai_details_used
      // You might want to update the state with these if they were AI-generated
      // Example: setAiDetails(resultData.ai_details_used);

      for (const card of resultData.generated_cards) {
        const MimeType = 'image/jpeg'; // Assuming JPEG as per backend
